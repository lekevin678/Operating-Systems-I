...
int getLength(char * fileName){
    ...
    int Length__file = ftell (f);   // get pointer length (number of character)
    Length__file--;                 // subtract one (ignore new line)
    fclose (f); 
...
int main(int argc, char *argv[]){
    ...
    while(keyContent[i] != '\0'){
        counter++;                      // count how man characters are in key
        i++;
    }
    if (length > counter){                          // if key is less than length, exit
        printf("key length is too short\n");
        exit(2);
    }    

    memset(buffer, '\0', sizeof(buffer));
    sprintf(buffer, "%c|%s|%s|%d", 'd' ,plainContent,keyContent,length); // create a buffer with handshake, text content, key content, and length 

    // Send message to server
    int totalSend=0;
    int bufferLenght = strlen(buffer);

    charsWritten = send(socketFD, &bufferLenght, sizeof(int), 0); // send length of entire buffer
    if (charsWritten < 0){
